...
author = opinion.select(
    "span.user-post__author-name").pop(0).get_text().strip()

rcmd = opinion.select(
    "span.user-post__author-recomendation > em").pop(0).get_text().strip()
stars = opinion.select(
    "span.user-post__score-count").pop(0).get_text().strip()
content = opinion.select(
    "div.user-post__text").pop(0).get_text().strip()
pros = opinion.select(
...
try:
    purchased = opinion.select(
        "div.review-pz").pop(0).get_text().strip()
except IndexError:
    purchased = None
...
try:
    ...
    purchaseDate = None
usefull = opinion.select(
    "span[id^=\"votes-yes\"]").pop(0).get_text().strip()
useless = opinion.select(
    "span[id^=\"votes-no\"]").pop(0).get_text().strip()
print(author, rcmd, stars, content, pros, cons, purchased,
      publishDate, purchaseDate, usefull, useless, sep="\n")
