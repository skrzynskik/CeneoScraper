...
plt.savefig(f"./figures/rcmd/{productId}_rcmd.png", bbox_inches="tight")
plt.close()

stars = opinions.stars.value_counts().reindex(np.arange(0,5.5,0.5), fill_value=0)
stars.plot.barh(color= "lightskyblue")
plt.title("Częstośc występowania poszczególnych ocen produktu w opiniach")
plt.xlabel("Liczba opinii")
plt.ylabel("Liczba gwiazdek")
plt.savefig(f"./figures/stars/{productId}_stars.png", bbox_inches="tight")
plt.close()
